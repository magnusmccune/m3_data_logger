...
inline bool tiny_code_reader_read(tiny_code_reader_results_t* results) {
    ...
    //
    // IMPORTANT: On ESP32/ESP8266, each Wire.requestFrom() starts a NEW I2C
    // transaction that resets the device's read pointer. This causes the same
    // bytes to be read repeatedly. ESP32 supports reading the full 256-byte
    // structure in a single transaction, so we use platform-specific chunking.
